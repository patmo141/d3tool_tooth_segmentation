...
class AITeeth_OT_send_cloud_convex_teeth(bpy.types.Operator):
    """Process convexified teeth shells in clod"""
    bl_idname = "ai_teeth.send_cloud_convex_teeth"
    bl_label = "Send Cloud Convex Teeth"
    ...
    def upload_job(self):
        ...
        teeth  = set([ob for ob in bpy.data.objects if ob.type == 'MESH' and "tooth" in ob.data.name and "Convex" not in ob.name])
        start = time.time()
...
class AITeeth_OT_cloud_convex_teeth_credit(bpy.types.Operator):
    """Submit file to get convex teeth"""
    bl_idname = "ai_teeth.cloud_convex_teeth"
    bl_label = "Cloud Convex Teeth"

    credits_cost =  FloatProperty(name = 'Available Credits', default = 3.5)
    ...
    def execute(self, context):
        if self.credits_avail < self.credits_cost:
            ...
            bpy.ops.ai_teeth.send_cloud_convex_teeth("INVOKE_DEFAULT")
...
def register():
    ...
    bpy.utils.register_class(AITeeth_OT_send_cloud_convex_teeth)
