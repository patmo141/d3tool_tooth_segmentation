...
class BMFacePatch(object):
    ...
    '''
    def __init__(self, bmface, local_loc, world_loc, vcol_layer, color = (1.0, .7, 0), label = ''):
    ...
    def uncolor_patch(self, color_layer = None):
        color = Color((1,1,1))
        if color_layer == None:
            color_layer = self.color_layer
    ...
    def find_boundary_edges(self, bme):
        ...
        #finish = time.time()
        #print('found islands if any in %f seconds' % (finish-start))
        ...
        finish = time.time()
        print('found boundary edges in % seconds ' % (finish-start))
        ...
        finish = time.time()
        print('sorted the boundary loops in %f seconds' % (finish-start))
...
class NetworkCutter(object):
    ...
    def create_spline_network_from_skeleton_patches(self, spline_net):  #maybe this should be a tool instead of datastructure
        '''
        This only creates new spline data from face patches that
        ...
        print('\n\n')
        print('CREATING SPLINES NETWORK FROM PATCHES')
        
        all_boundary_edges = set()
        
        for patch in self.face_patches:
            #if not patch.paint_modified: 
            ...
            all_boundary_edges.update(patch.boundary_edges)
            
        boundary_vs = set()
        for ed in all_boundary_edges:
            boundary_vs.update(ed.verts[:])
            
        nodes = [v for v in boundary_vs if sum([ed in all_boundary_edges for ed in v.link_edges]) > 2]  
        print("There are %i nodes" % len(nodes))
        #create the nodes, later connect them!
        ...
        for v in nodes:
            ...
            loc = v.co + .001 * delta
            no = v.normal
            new_pnt = spline_net.create_point(self.net_ui_context.mx * loc, loc, self.net_ui_context.mx_norm * no, f.index)    
            node_spline_map[v] = new_pnt
        ...
        def next_edge(v, prev_ed):
            ...
            return [ed for ed in v.link_edges if ed in all_boundary_edges and ed != prev_ed][0]
        
        vert_paths = []
        untested_nodes = node_set.copy()   
        while len(untested_nodes):
            v = untested_nodes.pop()
            
            branch_eds = [ed for ed in v.link_edges if ed.other_vert(v) in boundary_vs]
            if not len(branch_eds): continue
            for ed in branch_eds:
                #walk along long each of the brancehs
                branch = [v]
                next_v = ed.other_vert(v)
                next_ed = next_edge(next_v, ed)
                
                while next_v not in node_set:
                    branch += [next_v]
                    
                    if next_v in boundary_vs:
                        boundary_vs.remove(next_v)
                ...
                branch += [next_v]  #capture the last node
                vert_paths.append(branch)
        
        new_segs = []
        for vpath in vert_paths:
            raw_boundary = [v.co for v in vpath]
            relaxed_boundary = relax_vert_chain(raw_boundary, in_place = False)
            feature_inds = simplify_RDP(raw_boundary, .075)
            simple_path = [self.net_ui_context.mx * raw_boundary[i] for i in feature_inds]
            
            if len(simple_path) == 2:
                p0 = node_spline_map[vpath[0]]
                p1 = node_spline_map[vpath[-1]]
                
                seg = SplineSegment(p0, p1)
                new_segs += [seg]
                continue
            
            p0 = node_spline_map[vpath[0]]
            pf = node_spline_map[vpath[-1]]
            
            prev_pnt = p0
            new_points = []
            for pt in simple_path[1:len(simple_path)-2]:
    ...
    def create_spline_network_from_face_patches(self, spline_net):  #maybe this should be a tool instead of datastructure
        ...
        for patch in self.face_patches:
            ...
            feature_inds = simplify_RDP(raw_boundary, .25)
            simple_path = [self.net_ui_context.mx * raw_boundary[i] for i in feature_inds]
...
class SplineSegment(object): #NetworkSegment
    ...
    def tessellate(self):
        ...
        self.cb.tessellate_uniform(lambda p,q:(p-q).length, split=40)  #INITIAL TESSELATION
...
class SplineNetwork(object): #InputNetwork
    ...
    def connect_points(self, p1, p2, make_path=True):
        ''' connect 2 points with a segment '''
        if p1 == p2:
            print('cannot connect points to themselves')
            return None
