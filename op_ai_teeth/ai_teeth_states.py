...
these are the states and substates (tool states)
    ...
    main --> salience_edit                         (automatically switch to default tool)
        salience_edit <--> salience_paint (painting mode) (Now converted to sculpt mask mode)
        salience_edit <--> dilate/erode tool
        salience_edit --> Skeletonize Operator 
     \/ 
     \/  
      
    merge patches:   allow joining of patches
    
    \/ 
    \/  
     
    paint/adjust patches: Allow refinement of patches
...
class AITeeth_States():
    # salience_edit, spline, seed, and region are tools with their own states
    salience_edit_fsm = FSM()
    spline_fsm = FSM()
    seed_fsm   = FSM()
    region_fsm = FSM()
    ...
    def fsm_setup(self):
        ...
        self.salience_edit_fsm.set_call_args(self)
    ...
    def main(self):
        #dilate, erode one time.
        self.dilate_salience_verts()
        self.erode_salience_verts()
        self.remove_seeds_from_salience()
        self.remove_salience_vert_islands()
        return 'salience_edit'


    ####################################3
    #   Allow user to paint/delete, dilate/erode salience 
    ####################################
    @CookieCutter.FSM_State('salience_edit', 'enter')
    def salience_edit_enter(self):
        self.salience_edit_fsm.reset()

    @CookieCutter.FSM_State('salience_edit', 'exit')
    def salience_edit_exit(self):
        #maybe this needs to happen on paint enter...yes?
        self.salience_edit_fsm.reset()

    @CookieCutter.FSM_State('salience_edit')
    def salience_edit(self):
        return self.common(self.salience_edit_fsm)

    @CookieCutter.FSM_State('salience_edit', 'can exit')
    def salience_edit_can_exit(self):
        # exit salience_edit mode if we have skeletonized the baked_salience verts
        print('check exit')
        return self.has_skeletonized
    ...
    #######################################
    #The seed election state is not necessary for this tool
    #######################################
    ...
    #######################################
    #The Segmentation state will be different
    #######################################
    ...
    ####################################################
    #  salience_editing
    @salience_edit_fsm.FSM_State('main', 'enter')
    def saliencee_main_enter(self):
        
        self.brush = self.PaintBrush(self.net_ui_context, radius=self.brush_radius)
        self.ui_text_update()

    @salience_edit_fsm.FSM_State('main')
    def saliencee_main(self):
    ...
    @salience_edit_fsm.FSM_State('paint', 'can enter')
    def saliencee_paint_can_enter(self):
        #any time really, may require a BVH update if
        #network cutter has been executed
        return True

    @salience_edit_fsm.FSM_State('paint', 'enter')
    def saliencee_paint_enter(self):
        #set the cursor to to something
        self.click_enter_salience_paint()
        self.last_loc = None
        self.last_update = 0
        self.paint_dirty = False

    @salience_edit_fsm.FSM_State('paint')
    def saliencee_region_paint(self):
        self.cursor_modal_set('PAINT_BRUSH')
    ...
    @salience_edit_fsm.FSM_State('paint', 'exit')
    def saliencee_region_paint_exit(self):
        #self.brush.absorb_geom_geodesic(self.context, self.actions.mouse)
        print('REGULAR PAINT EXIT WORKING')
        self.salience_paint_confirm()
        self.net_ui_context.bme.to_mesh(self.net_ui_context.ob.data)


    @salience_edit_fsm.FSM_State('paint delete', 'enter')
    def saliencee_region_unpaint_enter(self):
        print('PAINT DELETE ENTER WORKING?')
        self.click_enter_salience_paint(delete = True)
        self.last_loc = None
        self.last_update = 0
        self.paint_dirty = False

    @salience_edit_fsm.FSM_State('paint delete')
    def saliencee_region_unpaint(self):
        self.cursor_modal_set('PAINT_BRUSH')
    ...
    @salience_edit_fsm.FSM_State('paint delete', 'exit')
    def saliencee_region_unpaint_exit(self):
        #self.brush.absorb_geom_geodesic(self.context, self.actions.mouse)
        print('PAINT DELETE EXIT WORKING')
        self.salience_paint_confirm_subtract()
        self.net_ui_context.bme.to_mesh(self.net_ui_context.ob.data)
    ...
    def region_paint_exit(self):
        self.brush.absorb_geom_geodesic(self.context, self.actions.mouse)
        if self.paint_mode == 'MERGE':
            self.paint_confirm_mergey()
        else:
            self.paint_confirm_greedy()
