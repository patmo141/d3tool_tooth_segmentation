...
from .ai_teeth_datastructure import InputPoint, CurveNode, SplineSegment
...
class AITeeth_UI_Draw():
    ...
    def draw_postview(self):
        ...
        if self._state in {'region','salience_edit'}:
    ...
    def draw_2D_stuff(self):
        ...
        if self._state == 'spline' and self.net_ui_context.snap_element and self.net_ui_context.selected:
            ...
            if psnap != psel:
                if isinstance(psel, InputPoint) and isinstance(psnap, InputPoint):
    ...
    def draw_3D_stuff(self):
        ...
        if self._state == 'salience_edit':
            if len(self.skeleton):
                draw3d_points(self.skeleton_points, blue2, 2, view_loc, view_ortho)
        ...
        if len(self.bad_patches):
            for p in self.bad_patches:
                self.draw_circle( p.world_loc, 50, .5, red, orange)
