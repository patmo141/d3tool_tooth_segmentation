...
def flood_selection_edge_loop_report(bme, edge_loop, seed_face, max_iters = 1000):
    '''
    bme - bmesh
    edge_loop - should create a closed edge loop to contain "flooded" selection
    if an empty set, selection will grow to non manifold boundaries
    ...
    return - set of faces, #of iterations, a dictionary mapping the iteration at each face
    '''
    
    edge_levy = set([e for e in edge_loop])
    topo_dict = {}
    topo_dict[seed_face] = 0
    ...
    return total_selection, iters, topo_dict
...
def decrease_vert_selection(selected_verts, iterations = 1):
    '''
...
def increase_vert_selection(selected_verts, iterations = 1):
    '''
    remove outer layer of selection
