...
class AITeeth_OT_tooth_extract_popup(bpy.types.Operator):
    """Mark Teeth for Extraction"""
    bl_idname = "ai_teeth.mark_extracted_teeth"
    bl_label = "Mark Extracted Teeth"
    ...
    def check(self, context):
        ...
        for ob in self.upper_teeth + self.lower_teeth:
            if (ob.get('EXTRACT') == True) != ob.select:
                ob['EXTRACT'] = ob.select
...
def register():
    bpy.utils.register_class(AITeeth_OT_tooth_extract_popup)
    

def unregister():
    bpy.utils.unregister_class(AITeeth_OT_tooth_extract_popup)