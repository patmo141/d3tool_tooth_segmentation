...
class AITeeth_OT_tooth_prep_popup(bpy.types.Operator):
    """Mark Teeth for Extraction"""
    bl_idname = "ai_teeth.mark_prepared_teeth"
    bl_label = "Mark Prepared Teeth"
    ...
    def check(self, context):
        ...
        for ob in self.upper_teeth + self.lower_teeth:
            ...
            if (ob.get('PREPARED') == 1) != ob.select:
                ob['PREPARED'] = ob.select
    ...
    def invoke(self, context, event):
        ...
        for ob in self.upper_teeth + self.lower_teeth:
            if ob.get('PREPARED') == None:
                #create the custom ID prop
                ob['PREPARED'] = False
                
            else:
                print('ALREADY HAS IT')
                ob.hide = ob.get('PREPARED') == 1
                ob.select = ob.get('PREPARED') == 1
                
            if ob.get('EXTRACT') != None:
                ob.hide = ob.get('EXTRACT') == True
...
def register():
    bpy.utils.register_class(AITeeth_OT_tooth_prep_popup)
    

def unregister():
    bpy.utils.unregister_class(AITeeth_OT_tooth_prep_popup)